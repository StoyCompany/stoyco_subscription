...
class PartnerProfileRepository with MultiContentAccessValidatorMixin {
  ...
  getLowestPricePlanByPartner({required String partnerId}) async {
    try {
      final LowestPricePlanResponseModel data = await _dataSource
          .getLowestPricePlanByPartner(partnerId);
  ...
  Future<Either<Failure, SubscriptionIsActiveResponse>> getLastUserPlanByPartner({
  ...
  Future<Either<Failure, GetCulturalAssetsResponse>> getCulturalAssetsByCommunityOwner({
    required String partnerId,
  }) async {
    try {
      final GetCulturalAssetsResponse data = await _dataSource.getCulturalAssetsByCommunityOwner(partnerId);
      data.copyWith(
